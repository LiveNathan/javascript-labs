...
const calculateTotalExpensesByYear = (year, expenses) => {
    return expenses
        .filter(expense => expense[0] === year)
        .reduce((total, expense) => total + Number(expense[3]), 0);
}

const totalExpenses2017 = calculateTotalExpensesByYear("2017", expenses);
const totalExpenses2018 = calculateTotalExpensesByYear("2018", expenses);

// 2. Output the total amount paid for Groceries.

const calculateTotalExpensesByCategory = (category, expenses) => {
    return expenses
        .filter(expense => expense[2] === category)
        .reduce((total, expense) => total + Number(expense[3]), 0)
}
const totalExpensesGroceries = calculateTotalExpensesByCategory("Groceries", expenses);

//   3. Output the total difference in each account after all transactions. So if $100 was deposited into the account and $50 spent, then the total change would be $50.
let accountBalances = {};
const calculateAccountBalances = (expenses) => {
    // Skip header row.
    for (let i = 1; i < expenses.length; i++) {
        let accountNumber = expenses[i][5];
        let amount = Number(expenses[i][3]);

        // Ensure the account number is a non-empty string and the amount is a valid number.
...
const eatingOut = expenses
    .filter(expense => expense[2] === 'Eating Out')
    .map(expense => [expense[0], expense[1], expense[3]]);

// 5. Create another array with only the date, description and amount of rows that have the category "Gear and Clothing".
const gearClothing = expenses
    .filter(expense => expense[2] === 'Gear and Clothing')
    .map(expense => [expense[0], expense[1], expense[3]]);